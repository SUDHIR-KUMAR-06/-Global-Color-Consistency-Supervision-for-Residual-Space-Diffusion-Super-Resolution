...
to $0.15$--$0.26$ across five paired runs spanning two datasets and an $8\times$
...
\begin{equation}
\hat r_0 = \big(r_t - \sqrt{1-\bar\alpha_t}\,\epsilon_\theta(r_t,t,c)\big)\big/\sqrt{\bar\alpha_t},
\label{eq:x0}
\end{equation}
...
Two implementation details are load-bearing. Eq.~\ref{eq:x0} must be evaluated in
...
$0.0022$ evaluation noise floor. Whether a residual diffusion model drifts in
color is therefore substantially a matter of chance.
...
\bottomrule
\end{tabular}
\caption{Color error across five paired runs spanning two datasets and an
...
within $0.15$--$0.26$ in every run, with a $6.5\times$ smaller standard deviation
and a $6.2\times$ narrower range. We describe this as \emph{bounding} color drift
rather than reducing it, and are deliberate about the strength of the claim: five
paired runs establish a consistent stabilising effect in our experiments, not a
formal guarantee. The benefit on any given run is governed by how far its baseline
