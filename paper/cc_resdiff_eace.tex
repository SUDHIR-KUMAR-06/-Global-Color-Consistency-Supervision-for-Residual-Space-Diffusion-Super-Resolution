% =====================================================================
% CC-ResDiff -- EACE-2026 submission version.
%
% CONFERENCE RULES, read off https://www.psit.ac.in/eace2026/ on 2026-09-12:
%   * "Papers must not exceed 10 pages, include no more than 8 keywords, and
%     follow the prescribed format: Times New Roman, 10-point font, double line
%     spacing, submitted in Microsoft Word (.doc/.docx) format."
%   * Springer LNEE formatting guidelines; Word template splnproc1703.docm.
%   * Submission deadline 15 Sep 2026, via
%     https://proconf.org/events/index.php?url=eace-2026
%     Notification 25 Sep; camera-ready 30 Sep; conference 20-21 Nov 2026.
%   * The first/corresponding author must register the paper, or it is
%     withdrawn from the proceedings.
%
% SPRINGER TEMPLATE HEADER ORDER (from splnproc1703.docm, styles in brackets):
%   [papertitle]  Contribution Title
%   [author]      First Author(1)[ORCID] and Second Author(2)[ORCID]
%   [address]     one line per affiliation, e-mail on the last line
%   [abstract]    prefixed "Abstract.", 150-250 words
%   [keywords]    prefixed "Keywords:", comma separated, closing period
%   Only heading levels 1-2 are numbered; levels 3-4 are unnumbered run-in
%   headings. References are numbered, cited as [1], in Springer style:
%   "Author, F.: Article title. Journal 2(5), 99-110 (2016)."
%   Current state: abstract 248 words, 8 keywords -- both within limits.
%
% This .tex is the CONTENT SOURCE, not the submission artefact: EACE accepts
% Word only. Paste this content into splnproc1703.docm and apply the ribbon
% styles above. paper/cc_resdiff.tex remains the extended draft.
%
% STILL OPEN:
%  1. Affiliations for both authors (marked TODO below), and confirmation of
%     the second author's display name -- it is inferred from the e-mail.
%  2. Verify every bibliography entry. They were drafted from memory; venues,
%     years and page numbers have NOT been checked against the actual papers.
%  3. Spacing conflict: the call for papers says double-spaced, but the
%     Springer LNEE template it also mandates is single-spaced. Ask the
%     organisers which governs the 10-page count -- it roughly halves what
%     fits. \doublespacing below is commented out so both can be measured.
% =====================================================================
\documentclass[10pt]{article}
% Springer's page geometry, read off the template's own sectPr: A4 with a
% 12.2 x 19.3 cm text block. Without this the LaTeX PDF paginates differently
% from the Word file and is useless as a proof of length.
\usepackage[a4paper,textwidth=12.2cm,textheight=19.3cm,top=5.2cm,left=4.4cm]{geometry}
\usepackage{times}          % Times New Roman, per the call for papers
\usepackage{setspace}
% \doublespacing            % uncomment to measure against the double-spaced rule
\usepackage{amsmath,amssymb}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage[hidelinks]{hyperref}
\newcommand{\best}[1]{\textbf{#1}}
% ORCID ids print as a superscript bracket in LaTeX; tex2docx.py maps them onto
% the template's ORCID character style. They are not printed in the book -- in
% the eBook they become links to the ORCID profile.
\newcommand{\orcid}[1]{\textsuperscript{[#1]}}

\title{\textbf{CC-ResDiff: Bounding Seed-Dependent Color Drift in
Residual-Space Diffusion Super-Resolution}}
% Both authors are at PSIT Kanpur, so Springer's convention collapses the
% affiliation to one unnumbered address line and drops the superscript numerals.
% Sudhir Kumar is first and corresponding author; EACE requires the
% corresponding author to register the paper or it is withdrawn.
% Job titles are not part of a Springer author block, so "Assistant Professor"
% is deliberately not printed. To name a department, put it before the
% institution: "Department of ..., Pranveer Singh Institute of Technology, ...".
% \normalsize because article's \maketitle sets authors at \large (12pt) and the
% names plus both ORCIDs then overrun the 12.2 cm column; Springer's own author
% style is 10pt. tex2docx.py strips the size command, so Word is unaffected.
\author{\normalsize Sudhir Kumar\orcid{0009-0007-0878-0231} and
Sanjoy Chattopadhyay\orcid{0009-0008-0937-6676}\\[4pt]
\small Pranveer Singh Institute of Technology, Kanpur 209305, Uttar Pradesh, India\\[2pt]
\small \texttt{snghsudhirkumar06@gmail.com} \quad
\small \texttt{sanjoyrsearch@gmail.com}}
\date{}
...
what makes Eq.~\ref{eq:gcc} complementary rather than redundant: at $k=8$ it
...
\small
% Springer reference style: "Surname, I.: Title. Venue vol(issue), pages (year)".
% Every entry below is transcribed from paper/references.bib, which the authors
% took from the publishers' own records. Author lists run to three names then
% "et al.". Do not edit these by hand -- correct references.bib and copy across.
\bibitem{shang2024resdiff} Shang, S., Shan, Z., Liu, G., et al.: ResDiff: combining CNN and diffusion model for image super-resolution. In: Proceedings of the 38th AAAI Conference on Artificial Intelligence, article 998, 9 pp. AAAI Press (2024)
\bibitem{li2022srdiff} Li, H., Yang, Y., Chang, M., et al.: SRDiff: single image super-resolution with diffusion probabilistic models. Neurocomputing 479, 47--59 (2022)
\bibitem{saharia2022image} Saharia, C., Ho, J., Chan, W., et al.: Image super-resolution via iterative refinement. IEEE Trans. Pattern Anal. Mach. Intell. 45(4), 4713--4726 (2023)
\bibitem{ho2020denoising} Ho, J., Jain, A., Abbeel, P.: Denoising diffusion probabilistic models. In: Advances in Neural Information Processing Systems 33 (NeurIPS 2020), article 574, 12 pp. Curran Associates (2020)
\bibitem{nichol2021improved} Nichol, A., Dhariwal, P.: Improved denoising diffusion probabilistic models. arXiv:2102.09672 (2021)
\bibitem{choi2022perception} Choi, J., Lee, J., Shin, C., et al.: Perception prioritized training of diffusion models. arXiv:2204.00227 (2022)
\bibitem{hang2023efficient} Hang, T., Gu, S., Li, C., et al.: Efficient diffusion training via Min-SNR weighting strategy. arXiv:2303.09556 (2024)
\bibitem{ledig2017photo} Ledig, C., Theis, L., Huszar, F., et al.: Photo-realistic single image super-resolution using a generative adversarial network. arXiv:1609.04802 (2017)
\bibitem{wang2018esrgan} Wang, X., Yu, K., Wu, S., et al.: ESRGAN: enhanced super-resolution generative adversarial networks. In: Computer Vision -- ECCV 2018 Workshops, Part V, pp. 63--79. Springer, Berlin (2018)
\bibitem{reinhard2001color} Reinhard, E., Ashikhmin, M., Gooch, B., et al.: Color transfer between images. IEEE Comput. Graph. Appl. 21(5), 34--41 (2001)
\bibitem{pitie2005ndim} Piti\'e, F., Dahyot, R.: N-dimensional probability density function transfer and its application to colour transfer. In: Proceedings of the IEEE International Conference on Computer Vision, vol. 2, pp. 1434--1439 (2005)
\bibitem{barron2015convolutional} Barron, J.T.: Convolutional color constancy. In: Proceedings of the IEEE International Conference on Computer Vision (ICCV), pp. 379--387 (2015)
\bibitem{afifi2019color} Afifi, M., Price, B., Cohen, S., et al.: When color constancy goes wrong: correcting improperly white-balanced images. In: Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), pp. 1535--1544 (2019)
\bibitem{heusel2017gans} Heusel, M., Ramsauer, H., Unterthiner, T., et al.: GANs trained by a two time-scale update rule converge to a local Nash equilibrium. In: Advances in Neural Information Processing Systems 30 (NIPS 2017), pp. 6629--6640. Curran Associates (2017)
\bibitem{zhang2018unreasonable} Zhang, R., Isola, P., Efros, A.A., et al.: The unreasonable effectiveness of deep features as a perceptual metric. In: Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR), pp. 586--595 (2018)
\bibitem{wang2004image} Wang, Z., Bovik, A.C., Sheikh, H.R., et al.: Image quality assessment: from error visibility to structural similarity. IEEE Trans. Image Process. 13(4), 600--612 (2004)
\bibitem{liu2015faceattributes} Liu, Z., Luo, P., Wang, X., et al.: Deep learning face attributes in the wild. In: Proceedings of the IEEE International Conference on Computer Vision (ICCV), pp. 3730--3738 (2015)
\bibitem{agustsson2017ntire} Agustsson, E., Timofte, R.: NTIRE 2017 challenge on single image super-resolution: dataset and study. In: Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition Workshops (CVPRW), pp. 1122--1131 (2017)
\end{thebibliography}
