...
% Author order as instructed: Sudhir Kumar first, chattopadhyaysanjoy18@gmail.com
% second. NOTE: the second author's display name below is INFERRED from that
% e-mail address and must be confirmed with them before submission, along with
% both affiliations and the preferred spelling of each name.
% EACE requires the first/corresponding author to register the paper.
\author{Sudhir Kumar$^{1}$ and Sanjoy Chattopadhyay$^{2}$\\[4pt]
\small $^{1}$TODO: Department, Institution, City, Country\\
\small $^{2}$TODO: Department, Institution, City, Country\\[2pt]
