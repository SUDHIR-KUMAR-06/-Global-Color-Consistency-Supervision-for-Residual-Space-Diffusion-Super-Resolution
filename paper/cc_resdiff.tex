...
inference, and reduces color drift by $63\%$ on average in the regime where that
drift occurs, on $99.3\%$ of test images, and beyond what any admissible post-hoc
correction can achieve.

Two results were not what we expected. An auxiliary loss defined on the
reconstructed $x_0$ inherits an extreme implicit timestep weighting, and ablating
that weighting showed it to be the source of the method's benefit rather than a
flaw---supervision of global color must act at large $t$, where global structure
is decided. And our mechanism-based explanation for the accompanying FID
regression was falsified by the same experiment, leaving that regression an open
question.

The negative DIV2K result bounds the claim: this is a targeted remedy for an
identifiable failure mode, not a universal improvement. Predicting in advance
which training configurations will drift, and testing whether the benefit grows
in the more severely under-trained regime ResDiff describes, are the natural next
steps.
