...
configurations, whereas the GCC loss confines it to $0.15$--$0.24$---a standard
deviation $8\times$ smaller. The loss is therefore better described as
...
to generalise. A gradient-level analysis shows the term's influence is not
...
\midrule
CelebA, seed 1234  & 0.8234 & 0.2423 & $-70.6\%$ \\
CelebA, seed 5678  & 0.3878 & 0.2061 & $-46.9\%$ \\
CelebA, seed 2024  & 0.1397 & 0.1461 & $+4.6\%$ \\
DIV2K              & 0.1650 & 0.2036 & $+23.4\%$ \\
\midrule
\emph{std across runs} & \emph{0.317} & \emph{0.040} & \\
\bottomrule
\end{tabular}
\caption{Per-run color error. The baseline's drift is highly seed-dependent,
spanning $0.14$--$0.82$; CC-ResDiff lands near $0.20$ regardless. The benefit on
any given run is therefore a function of how much that run would otherwise have
drifted.}
\label{tab:perseed}
\end{table}
...
to $0.82$ across otherwise identical configurations. CC-ResDiff's is not: it
falls between $0.15$ and $0.24$ in every run, with a standard deviation $8\times$
smaller than the baseline's. The apparent size of the improvement on any
individual run is therefore governed by how badly that run happened to drift.
...
The DIV2K result is therefore better understood as a fourth data point consistent
with the bounding behaviour of Section~\ref{sec:main} than as a failure to
