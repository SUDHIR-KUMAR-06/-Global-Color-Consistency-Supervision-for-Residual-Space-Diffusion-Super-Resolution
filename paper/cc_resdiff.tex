...
rewards while leaving per-image LPIPS largely indifferent. The reweighting of
Section~\ref{sec:weighting} is the direct test of that prediction: if the skew
causes the regression, cancelling it should remove the regression.

More generally, this is a property of \emph{any} auxiliary loss defined on the
reconstructed $x_0$ rather than of ours specifically. Such a term inherits the
$1/\sqrt{\bar\alpha_t}$ factor and is therefore implicitly weighted towards large
$t$ by orders of magnitude, whether or not its designer intended it. We recommend
reporting the gradient ratio, not the loss ratio, whenever such a term is
introduced.
